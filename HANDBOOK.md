...
## 5. Developer Guide & Installation

ObsidianQ is designed to be completely invisible to your application logic once installed. You continue to use the standard `java.security` and `javax.crypto` interfaces you already know.
...
## 6. ASN.1 JCA Compliance

As of `v1.0.0`, all Keys generated by ObsidianQ natively support strict ASN.1 DER encodings when calling `getEncoded()`. This allows drop-in compatibility with:
- Standard Java KeyStores (JKS, PKCS12)
- Hardware Security Modules (HSM)
- TLS 1.3 Post-Quantum extensions (e.g. BouncyCastle interop)

- **KyberPublicKey** emits `SubjectPublicKeyInfo` (X.509 format, 1206 bytes)
- **KyberPrivateKey** emits `PrivateKeyInfo` (PKCS#8 format, 2424 bytes)

The NIST CSOR OID used is `2.16.840.1.101.3.4.4.2`.

---

## 7. Java 21 Integration (`javax.crypto.KEM`)
